...
**常大校园快递收发系统** 是一个现代化校园快递管理平台，旨在解决校内快递收发效率低、信息不透明等问题。本项目为**学校实训项目**，采用前后端分离架构，模拟真实产品开发流程，涵盖了从需求分析、系统设计到编码实现的全过程。
...
### 前端技术
- **Vue 3**: 使用 Composition API 进行逻辑复用。
- **TypeScript**: 强类型约束，提高代码质量。
- **Vite**: 极速构建工具。
- **Pinia**: 现代化状态管理库。
- **Vue Router 4**: 路由管理。
- **CSS3**: Flexbox/Grid 布局，Backdrop-filter 模糊效果，CSS 动画。
